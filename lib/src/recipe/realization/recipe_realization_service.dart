...
void saveRecipeRealization(
    RecipeRealization realization, BuildContext context) {
...
void updateRecipeRealizationCurrentTask(int currentTask, BuildContext context) {
  var database = getDatabase(context);
  var entityCompanion =
      RecipeRealizationEntityCompanion(currentTask: currentTask.toDriftValue());
  database.recipeRealizationDao.updateRecipeRealization(1, entityCompanion);
}

Future<RecipeRealization> getRecipeRealizationById(
    int id, BuildContext context) {
  var database = getDatabase(context);
  return database.recipeRealizationDao
      .recipeRealizationById(id)
      .map((data) => RecipeRealization(data.currentTask, data.recipe));
}
